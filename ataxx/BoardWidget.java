...
class BoardWidget extends Pad  {
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (int i = 0; i < SQDIM * SIDE; i += SQDIM) {
            g.setColor(LINE_COLOR);
            g.setStroke(LINE_STROKE);
            g.drawLine(i, 0, i, _dim);
            g.drawLine(0, i, _dim, i);
        }
        for (int pos = begPlayBoard;
             pos < endPlayBoard; pos += 1) {
            if (_model.get(pos) == RED) {
                g.setColor(RED_COLOR);
                g.setStroke(BLOCK_STROKE);
                g.fillOval((pos % 11 - 2) * SQDIM + 10,
                        (pos / 11 - 2) * SQDIM + 10, PIECE_RADIUS * 2,
                        PIECE_RADIUS * 2);
            } else if (_model.get(pos) == BLUE) {
                g.setColor(BLUE_COLOR);
                g.setStroke(BLOCK_STROKE);
                g.fillOval((pos % 11 - 2) * SQDIM + 10,
                        (pos / 11 - 2) * SQDIM + 10, PIECE_RADIUS * 2,
                        PIECE_RADIUS * 2);
            } else if (_model.get(pos) == BLOCKED) {
                g.setColor(BLOCK_COLOR);
                g.setStroke(BLOCK_STROKE);
                drawBlock(g,  (pos % 11 - 2) * SQDIM,
                        (pos / 11 - 2) * SQDIM);
            }
        }
    ...
    void drawBlock(Graphics2D g, int cx, int cy) {
        g.setColor(BLOCK_COLOR);
        g.setStroke(BLOCK_STROKE);
        g.drawRect(cx + 5, cy + 5, BLOCK_WIDTH, BLOCK_WIDTH);
    }
    ...
    private void handleClick(String unused, MouseEvent where) {
        ...
        if (where.getButton() == MouseEvent.BUTTON1) {
            ...
            int distRowMid = mouseRow - '4';
            char realmouseRow = (char) (mouseRow - 2 * distRowMid);
                ...
                if (_blockMode) {
                    ...
                    if (_selectedCol != 0) {
                        _commandQueue.offer(String.valueOf(_selectedCol)
                                + (_selectedRow)
                                + "-" + (mouseCol) + (realmouseRow));
                        _selectedCol = _selectedRow = 0;
                    } else {
                        _selectedCol = mouseCol;
                        _selectedRow = realmouseRow;
                    }
                }
            }
        }
